  ...
  <>
   ...
   <nav className={`w-full fixed px-5 lg::px-8 xl:px-[8%] py-4 flex items-center justify-between z-50 ${isScroll ? "bg-white bg-opacity-50 backdrop-blur-lg shadow-sm dark:bg-darkTheme dark:shadow-white/20" : ""}`}>
    ...
    <a href="#top">
     ...
     <span className='mulandi'>Mulandi</span>
     <span className='red-dot'>.</span>
    </a>
    <ul className={`hidden md:flex items-center item-center gap-6 lg:gap-8 rounded-lg px-12 py-3 ${isScroll ? "":" bg-white shadow-sm bg-opacity-50 dark:border dark:border-white/50 dark:bg-transparent"}`}>
     <NavbarLinks />
    </ul>
    ...
    {/* --- mobile menu --- */}
    <ul ref={sideMenuRef} className='flex md:hidden flex-col gap-4 py-20 px-10 fixed -right-64 -64 top-0 bottom-0 w-64 z-50 h-screen bg-rose-50 transition duration-500 dark:bg-darkHover dark:text-white '>
